...
date_default_timezone_set('America/Sao_Paulo');
		...
		$data = date("Y_m_d");
		$sit_mnt = utf8_decode(trim($_POST['sit_mnt']));
			...
			$sql = mysqli_query($link, "UPDATE 2becmb_material SET sit_mnt='$sit_mnt' WHERE id LIKE '$id'");
			...
			if (!$sql){
					...
					echo "
                           <META HTTP-EQUIV=REFRESH CONTENT='0; URL=2becmb_baltera_sit_mnt.php'>
			...
			$comando = utf8_decode('Alterou a Situação de Manutenção do Material (UPDATE 2becmb_material SET sit_mnt='.$sit_mnt.' WHERE id LIKE '.$id.'');
