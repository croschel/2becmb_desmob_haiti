...
// consulta
	...
	$resultado = mysqli_fetch_assoc($query);
		...
		$ficha = utf8_encode($resultado['ficha']);
		$ppeb = utf8_encode($resultado['ppeb']);
?>

<form class="jotform-form" action="2becmb_balterar_motorista.php" method="post" name="teste"  enctype="multipart/form-data" id="" accept-charset="utf-8" novalidate="true">
	  ...
	  <li class="backcolor" data-type="control_text" id="id_56">
        <div id="cid_56" class="form-input-wide">
          <div id="text_56" class="form-html">
			...
			<tr><td><font color='#000000' face="verdana" size=2>Placa/Prefixo/EB:</font></td><td><input name="ppeb" type="text" id="ppeb" value="<?php echo $ppeb;?>" readonly></td></tr>
			...
			<tr><td><font color='#000000' face="verdana" size=2><b>Motorista: &nbsp </b></font><select name='motorista'><?php
  $motoristas = "SELECT * FROM usu WHERE cnh LIKE '2' OR cnh LIKE '3' OR cnh LIKE '4' OR cnh LIKE '5' ORDER by usuario ASC";
  $sql = mysqli_query($link, $motoristas);
  echo "<option value=''>Listar</option>";
  echo "<option value='-'>Sem Motorista</option>";
  while ($resultado2 = mysqli_fetch_array($sql)) {
  $motorista = $resultado2['usuario'];
  $motorista1 = utf8_encode($resultado2['nome_guerra']);
  $motorista2 = utf8_encode($resultado2['posto_grad']);
  echo "<option value='$motorista'>$motorista2 $motorista1</option>";
  }
  ?>
			</select></td></tr>
