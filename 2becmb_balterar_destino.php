...
date_default_timezone_set('America/Sao_Paulo');
		...
		$destino = utf8_decode(trim($_POST['destino']));

	
	
			// ALtera os dados no banco de dados
			
			$sql = mysqli_query($link, "UPDATE 2becmb_material SET destino='$destino' WHERE id LIKE '$id'");
			...
			if (!$sql){
					...
					echo "
                           <META HTTP-EQUIV=REFRESH CONTENT='0; URL=2becmb_baltera_destino.php'>
			...
			// Gravar na tabela auditoria (FIM)
                    ...
                    echo "
								...
								<script type=\"text/javascript\" charset=UTF-8>
								alert(\"A campo DESTINO foi alterado com sucesso! \");
                               </script>
							   <script>
