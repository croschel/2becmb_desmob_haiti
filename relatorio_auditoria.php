   ...
   exit;
	...
	if($nivel < 9){
					...
					echo "
                           ...
                           alert(\"O Sr não possui permissão de acesso a esta funcionalidade.\");
                           </script>
						   <script>
...
<form class="jotform-form" action="" enctype="multipart/form-data" method="post" name="busca" id="" accept-charset="utf-8" novalidate="true">
  ...
  <div class="form-all">
    <table align="center" border="0" width="100%" class="backcolor1">
		...
		<tr>
		<td><font color="#EEE9E9" size="4" face="arial">Desmobilização Mat Cl VI e IX do Haiti </font></td>
		<td align="center"> <font color="#EEE9E9" size="4" face="arial"><?php echo $posto_grad.' '.$nome_guerra.'('.$funcao.')';?></font></td>
		<td>
		<p align="right">
		
		<a href="/2becmb_desmob_haiti/index1.php" title="Página Inicial" > <img src="./imagens/home.png" width="40" height="40"></a> &nbsp&nbsp
...
//$link = mysqli_connect("localhost","root","","2becmb_desmob_haiti");
   ...
   if ($_POST['consulta']) {
	...
	$sql = "SELECT * FROM auditoria WHERE usuario LIKE '%$f_usuario' AND comando LIKE '%$f_comando%' AND om LIKE '%$f_om%' AND nome_guerra LIKE '%$f_nome_guerra' AND posto_grad LIKE '%$f_posto_grad' AND datahora_comando LIKE '%$f_datahora_comando%' ORDER BY $ord1, $ord2";
	...
	$sql_listar_total = mysqli_query($link, "SELECT * FROM auditoria"); //total de registros na tabela material
...
<tr bgcolor="#87CEFF" class="backcolor" align="center" height="40">
  ...
  $posto_grads = "SELECT DISTINCT posto_grad FROM auditoria ORDER BY posto_grad";
  ...
  $nome_guerras = "SELECT DISTINCT nome_guerra FROM auditoria ORDER BY nome_guerra";
  ...
  $usuarios = "SELECT DISTINCT usuario FROM auditoria ORDER BY usuario";
  ...
  $oms = "SELECT DISTINCT om FROM auditoria ORDER BY om";
...
<?php
//echo "<meta HTTP-EQUIV='refresh' CONTENT='20;URL=relatorio_auditoria.php'>";
?>
