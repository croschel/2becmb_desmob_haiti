...
<form class="jotform-form" action="" enctype="multipart/form-data" method="post" name="busca" id="" accept-charset="utf-8" novalidate="true">
  ...
  <div class="form-all">
    <table align="center" border="0" width="100%" class="backcolor1">
		...
		<td colspan="3"><font color='#000000' size="5" face="arial">Batalhão Cidade de Camoinas</font><hr width="100%" align="left"></td>
		</tr>
		<tr>
...
//$link = mysqli_connect("localhost","root","","2becmb");
   ...
   if ($_POST['consulta']) {
	...
	if ($_POST['consulta'])
	// consulta sem filtro
	{$sql1 = "SELECT * FROM 2becmb_material WHERE (sit_doc LIKE '%$f_sit_doc' $op doc_validade $op_reg '$hoje') AND classe LIKE '%$f_classe' AND emprego LIKE '%$f_emprego%' AND material LIKE '%$f_material%' AND tipo LIKE '%$f_tipo%' AND marca LIKE '%$f_marca%' AND ppeb LIKE '%$f_ppeb%' AND ppeb LIKE '%$busca%' AND modelo LIKE '%$f_modelo%' AND versao LIKE '%$f_versao%' AND serie LIKE '%$f_serie%' AND capacidade LIKE '%$f_capacidade%' AND ano LIKE '%$f_ano' AND sit_mnt LIKE '%$f_sit_mnt' ORDER BY marca, modelo";} else
...
<font color='#000000' face="arial" size=2>Placa/Prefixo/EB (ou parte)</font> <input name="busca" type="text" id="busca">
			
			
<input type="submit" name="consulta" value="Buscar">&nbsp&nbsp&nbsp&nbsp&nbsp
...
<tr bgcolor="#87CEFF" class="backcolor" align="center" height="40">
  ...
  <td width="60" ><font size=2 color="#000000" face="arial"><b>Doc</b><BR></font></td>
  
   <td width="60" ><font size=2 color="#000000" face="arial"><b>Mnt</b><BR></font></td>
...
<tr bgcolor="<?php print $cor;?>" >
  ...
  <td width="30" ><font size=2 color="#000000" face="arial"><img src="<?php echo './2becmb_foto/'.$foto.'.jpg';?>" width="45" height="60"></font></td>
  ...
  <td><p align="center"><?php
	    if (($doc == 0) && (($sit_doc == 0) || ($doc_validade < $hoje))) {echo '<a href="./2becmb_documento/sem_doc.jpg" target="_blank"><img src="./imagens/vencido_sem_doc.png" width="33" height="33" title="Documento validade: '.$doc_validade.'. Situação IRREGULAR. Documento não carregado"></a>';} 
	elseif (($doc == 1) && (($sit_doc == 0) || ($doc_validade < $hoje))) {echo '<a href="./2becmb_documento/doc_'.$id.'.jpg" target="_blank"> <img src="./imagens/doc1.png" width="33" height="33" title="Documento validade: '.$doc_validade.'. Situação IRREGULAR. Visualizar Documento"></a>';} 
	elseif (($doc == 0) && (($sit_doc == 1) && ($doc_validade >= $hoje))){echo '<a href="./2becmb_documento/sem_doc.jpg" target="_blank"><img src="./imagens/modelo.png" width="33" height="33" title="Documento válido até: '.$doc_validade.'. Situação REGULAR. Documento não carregado"></a>';}
	elseif (($doc == 1) && (($sit_doc == 1) && ($doc_validade >= $hoje))){echo '<a href="./2becmb_documento/doc_'.$id.'.jpg" target="_blank"><img src="./imagens/doc.png" width="33" height="33" title="Documento válido até: '.$doc_validade.'. Situação REGULAR. Visualizar Documento"></a>';}
    elseif ($sit_doc == 2){echo '<a href="./2becmb_documento/doc_'.$id.'.jpg" target="_blank"><img src="./imagens/providenciando.png" width="33" height="33" title="Documento validade: '.$doc_validade.'. Situação PROVIDENCIANDO. Visualizar Documento"></a>';}
...
<tr>
  <td colspan="6" bgcolor="<?php echo $cor1;?>"><font size=2 color="#000000" face="arial"><p align="center">A pesquisa retornou: [ <?php echo $encontrados; ?> ] resultado(s) em [ <?php echo $total ?> ] cadastrados. Resultado Percentual: [<?php $percentual = round($encontrados*100/$total,2);echo $percentual ?> %]</p></font></td>
